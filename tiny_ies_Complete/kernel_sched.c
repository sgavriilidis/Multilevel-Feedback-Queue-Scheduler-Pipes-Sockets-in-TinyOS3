...
TCB* spawn_thread(PCB* pcb, void (*func)())
{
 ...
 //////////////////////////////////////////////////////////////////////// 
  tcb->priority = 5;     //new threads get default priority level of 5
 ////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////
rlnode PriorityQueue[MAX_PRIORITY];   /* The scheduler queue */
/////////////////////////////////////////////////////////////////////
...
static void sched_queue_add(TCB* tcb)
{
  ...
  rlist_push_back(& PriorityQueue[tcb->priority], & tcb->sched_node);
...
static TCB* sched_queue_select()
{
  ...
  //we search all cells of the PriorityQueue array
  //and get the first thread of the first non-NULL list
  int i=0;                   
  while((i<MAX_PRIORITY-1) && is_rlist_empty(&PriorityQueue[i])) {
    i++;
  }
  /* Get the head of the SCHED list */
  rlnode * sel = rlist_pop_front(& PriorityQueue[i]);
...
{ 
  ...
  Mutex_Lock(& sched_spinlock);
  //////////////////////////////////////////////////////////////////////////////////
  Exitcount++ ;   //counter for yield calls
  if (Exitcount > MAX_COUNTER)
  {                                 //it boost the threads in small time laps to avoid starvation
    Exitcount = 0 ;              // Reset  Counter
    for (int i=1 ; i < MAX_PRIORITY ; i++)        //every 20 calls of yield 
      for (int j = 0 ; j < rlist_len(&PriorityQueue[i]) ; j ++ )  //we boost every queue by 1 level of priority
        rlist_push_back (&PriorityQueue[i-1],rlist_pop_front (&PriorityQueue[i]));
  }
 //we modify the value of current priority depending on the cause
  switch (cause)
  {
    case SCHED_QUANTUM: // The Quantum is depleted , so decrease priority
      if (current->priority < MAX_PRIORITY - 1 ) {
        current->priority = current->priority + 1;
      }
      break;
      
    case SCHED_IO: // The Quantum is not depleted
      if (current->priority > 0) {
          current->priority = current->priority - 1;
      }
      break;

    case SCHED_MUTEX: //the thread is called with mutex that is already taken by an other thread.decrease priority
      if(current->priority<MAX_PRIORITY-1) {
       // current->priority = MAX_PRIORITY-1;
        rlist_push_back (&PriorityQueue[MAX_PRIORITY-1],rlist_pop_front (&PriorityQueue[current->priority]));
        }
      break;
    default :
       break;
...
void gain(int preempt)
{
  ...
  if(preempt) preempt_on;     //dinei ti dunatotita na stamatisei mia diadikasia
                              //3ekinaei ton timer me xrono = quantum
...
void initialize_scheduler()
{
  ...
  for(int i=0;i<MAX_PRIORITY;i++) {
    rlnode_init(&PriorityQueue[i], NULL);
  }
