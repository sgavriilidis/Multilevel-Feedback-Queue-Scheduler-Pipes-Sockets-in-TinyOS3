...
typedef struct pipe_control_block
{
  ...
  FCB* reader; 
  FCB* writer; 
  CondVar hasData;  //for readers
  CondVar hasSpace ;//for writers
  int head ;
  int numOfElements ;
} PIPECB;
...
typedef struct peer_socket_control_block{
  PIPECB *pipe_send;  //where we write
  PIPECB *pipe_recv;  //from we read
  
}PEER_SCB;

typedef struct socket_control_block
{
  int refcount; // when a thread sleeps in connect
  FCB* fcb;    
  port_t port; 
  ...
  union 
  {
    LISTENER_SCB listener_socket;
    PEER_SCB peer_socket;
  };
} SCB;
typedef struct request_struct
{
  SCB* scb; // the socket that made the request 
  rlnode request_node;
  CondVar request_cv; 
  int accepted; 
} request;
